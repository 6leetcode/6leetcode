      ...
      <view class="cu-list menu">
        ...
        >
          <view class="content">
            <text
              class="text-grey"
            >{{ question.frontend_question_id }}. {{ question.translated_title }}</text>
      ...
      complete: content => {
        ...
        for (let i = 0; i < content.data.questions.length; i++) {
          if (content.data.questions[i].translated_title.length > 14) {
            content.data.questions[i].translated_title =
              content.data.questions[i].translated_title.substr(0, 14) + "...";
          }
          if (content.data.questions[i].frontend_question_id < 10) {
            content.data.questions[i].frontend_question_id =
              "000" + content.data.questions[i].frontend_question_id;
          } else if (content.data.questions[i].frontend_question_id < 100) {
            content.data.questions[i].frontend_question_id =
              "00" + content.data.questions[i].frontend_question_id;
          } else if (content.data.questions[i].frontend_question_id < 1000) {
            content.data.questions[i].frontend_question_id =
              "0" + content.data.questions[i].frontend_question_id;
          }
        }
